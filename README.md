...
**発生した問題**：キャラクターに `hue-rotate` フィルターを inline スタイルで指定したが、マイルームでは常にピンクのまま変わらなかった。

**原因の特定**：キャラクター画像には `@keyframes roomCharaSparkle` というグロウアニメーションが付いており、アニメーション内で `filter` プロパティを上書きしていた。CSS の仕様上、`@keyframes` によるスタイルは inline スタイルを含む他のすべての指定より優先されるため、hue-rotate が無効になっていた。

**解決策**：CSS のレンダリングモデルでは**親要素のフィルターは子の合成済み出力全体に後から適用される**。この仕様を利用し、フィルターをアニメーションのない親要素に移動した。結果として、アニメーション（グロウ効果）と色変換が干渉せず共存できるようになった。
